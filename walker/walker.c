...
void walk(Project *proj, char *path, int flags, int modes)
{
  ...
  if (S_ISDIR(st.st_mode))
  {
    ...
    {
      if (skip_dir(dir_content->d_name))
      {
        continue;
      }
      ...
      walk(proj, fullpath, flags, modes);
...
void add_file_to_proj(File file, Project *proj)
{
